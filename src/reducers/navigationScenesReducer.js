...
export default function NavigationScenesReducer (scenes, nextState, prevState) {
  if (prevState === nextState) return scenes
  ...
  nextState.routes.forEach((route, index) => {
    ...
    invariant(
      !nextKeys.has(key),
      `navigationState.routes[${index}].key "${key}" conflicts with another route!`
    )
    nextKeys.add(key)
  ...
  if (nextScenes.length !== scenes.length) return nextScenes

  if (nextScenes.some((scene, index) => !areScenesShallowEqual(scenes[index], scene))) return nextScenes
